...
class InventoryReconciliationApp:
    ...
    def _process_complete_transaction(self, data: Dict, transaction_type: str, parse_result: ParseResult):
        ...
        2. Extract clean data from Airtable using SKU as single source of truth
        3. Execute proper Zoho workflow (PO→Bill or SO→Invoice→Shipment)
        4. Send enhanced success notifications
        """
        ...
        try:
            ...
            if airtable_result.get('success'):
                ...
                if self.zoho.is_available:
                    ...
                    logger.warning("⚠️ Zoho unavailable - skipping sync")
    ...
    def _execute_zoho_workflow(self, airtable_result: Dict, transaction_type: str, transaction_record_id: str):
        ...
        try:
            ...
            logger.info(f"⏱️ Zoho workflow completed in {zoho_duration:.2f}s")
    ...
    def run(self) -> None:
        ...
        self.discord.send_info_notification(
            ...
            "System initialized with proper Purchase/Sales Order workflows",
            {
                ...
                "Auto Bills": "✅" if self.zoho.auto_create_bills else "❌",
                "Auto Invoices": "✅" if self.zoho.auto_create_invoices else "❌",
                "Auto Shipments": "✅" if self.zoho.auto_create_shipments else "❌",
                "Direct Adjustments": "❌ Disabled" if not self.zoho.allow_direct_adjustments else "⚠️ Enabled",
                "Gmail": "✅ Connected",
        ...
        try:
            ...
            if self.zoho.use_proper_workflows and hasattr(self.zoho, 'validate_inventory_adjustments_empty'):
                ...
                if adjustment_check.get('is_clean', True):
                    ...
                    logger.warning(f"⚠️ Found {auto_adjustments} auto-generated adjustments - should be zero with proper workflows")
            ...
            if hasattr(self.zoho, 'generate_inventory_sync_report'):
                ...
                if sync_report.get('discrepancies'):
                    logger.warning(f"⚠️ Found {len(sync_report['discrepancies'])} inventory discrepancies")
